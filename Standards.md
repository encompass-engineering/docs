...
## 2.1. Authoring Formats

1. YAML and Markdown are the two authoring formats — they MUST be interconvertible
1. Internal structured documentation: YAML is the default authoring format
1. External or user-facing documentation: Markdown is the default authoring format
1. Formal prose-heavy documentation: LaTeX (generated from YAML or Markdown source)
1. Informal prose-heavy documentation: Markdown

## 2.2. Output Formats

1. All of the following are valid output formats, generated from YAML or Markdown source:
   1. YAML
   1. Markdown
   1. JSON
   1. XML
...
1. Block scalars: `|` for literal (preserve newlines), `>` for folded (join lines)
1. Quoted strings: use quotes only when necessary (special characters, reserved words)
1. Flow syntax (`{key: value}`, `[a, b, c]`) is PROHIBITED — use block syntax only
1. Anchors and aliases (`&`/`*`) are PROHIBITED
1. Multi-document syntax (`---`) is PROHIBITED
   > Review: advanced YAML components (custom tags, merge keys)

## 2.4. Markdown Standards

1. Headings: MUST be numbered (e.g., `# 1.  Title`, `## 1.1. Subtitle`)
1. Lists: MUST be numbered using CommonMark `1.` syntax
1. Bullet lists (`-`) are PROHIBITED
1. Callouts are PROHIBITED
1. Mathematical expressions:
   1. Inline: `$\begin{equation}x^2 + 2x + 1\end{equation}$`
   1. Display: `$$\begin{equation}x^2 + 2x + 1\end{equation}$$`
   1. Display math MUST use `\begin{equation}` for numbering — bare `$$...$$` without equation environment is PROHIBITED
   1. Equation numbering enables cross-referencing and MUST be used for all display math

## 2.5. CSV Standards

1. Header records are PROHIBITED — CSV files contain data only
1. Schema MUST be defined externally (in a YAML or Markdown schema definition file)
1. The schema definition MUST specify: column order, column names, data types, and constraints

## 2.6. LaTeX Output Standards

1. Default document class: `report`
1. Section numbering: always enabled
1. Deeply nested content: MUST use nested `enumerate` environments
1. Document class selection by nesting depth:
   1. Depth ≤ 3: `article`
   1. Depth 4: `article` with `paragraph`
   1. Depth 5: `book`
   1. Depth ≥ 6: `book` with `part`

## 2.7. Special Cases

1. Documents that declare style constraints MUST be self-referential in style — they MUST follow the constraints they declare